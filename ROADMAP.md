...
**目的**
サイト・アプリ・デジタルコンテンツのコーディングを、なるべく広い範囲で疎結合な「ノード」として分散管理する。狙いは、**一度作ったノードを複数のコンテンツで使い回すことで、コーディングにかかるリソース(実装・レビュー・保守の手間)を継続的に減らしていく**こと。これにより:
...
**ノード化の対象範囲に限定は無い**。決済・認証・エラーハンドリングは最初の実装例に過ぎず、**複数回使われる可能性があるものは何でもノード化の候補になりうる**(UIコンポーネント、バリデーションロジック、通知処理、データ変換など)。「このジャンルはノード化しない」という線引きは設けない — 判断基準は「同じコードが2箇所以上で必要になったかどうか」のみであり、それ以外の性質(バックエンド/フロントエンド、参照型/生成型などの分類)は`node_type`/`category`で表現する。
...
- **参照型ノード(通常)**: 独自の`version`フィールドは持たない。submoduleのコミットハッシュがそのまま実バージョンであり、二重管理を避ける。
- **参照型ノード(2.12のバージョニング配信対象、non-security-sensitiveのみ)**: `schema.yaml`に`version`フィールドは追加しない(二重管理を避ける原則は維持)。代わりに**gitブランチ**でバージョンを表現する(2.12参照)。
...
- **この節の運用(手動トリガー)は、security-sensitiveなノードには今後も適用され続ける。** non-security-sensitiveなノードについては、2.12のバージョニング配信システムにより、承認をトリガーとした自動反映に置き換えることができる(security-sensitiveなノードにこの自動反映を適用しない理由も2.12参照)。
...
**2.12のバージョニング配信システムについての補足**: non-security-sensitiveなノードに限り、gitブランチによるバージョン管理+CIによる自動反映という仕組みを導入する。これはnpm package化ではなくgit標準機能(ブランチ・submodule)のみで実現するため、認証設定やpublish作業といったnpm特有の手間は生じない。ただし「node-library側が全content repoへの書き込み権限を持つ」という新しいインフラ(CI用の書き込みトークン)は必要になる点は、この比較表の「追加インフラ不要」という利点から部分的に外れることを認識しておく(2.12参照)。
...
### 2.12 ノード単位のバージョニングと配信(Public/Private承認モデル)

**背景**: Phase 1完了後、複数のcontent repoで実運用する段階になると、2.9の「常に手動トリガー」という反映方式は、プロジェクト数が増えるほど反映漏れ・反映忘れのコストが無視できなくなる。一方で、完全自動(承認すら無い即時反映)は、security-sensitiveなノードにおいて「誤りが全プロジェクトへ即座に伝播する」という2.9が避けようとしたリスクをそのまま抱えることになる。この対立を、**カテゴリによって挙動を分ける**ことで解消する。

**適用対象**: **non-security-sensitiveなノードのみ**。`payment`/`auth`/`secrets`等、security-sensitiveなノード(2.7参照)には適用しない。理由: security-sensitiveな処理は、たとえコードをダウングレードで元に戻せても、露出していた間に発生した実害(不正な取引、不正アクセスによるデータ漏洩など)そのものは取り消せないため。ダウングレードは「不具合の是正」はできても「セキュリティ被害の防止」はできない。security-sensitiveなノードは引き続き2.9の手動反映モデルを使う。

**設計方針**: `schema.yaml`に`version`フィールドは追加しない(2.1・2.6の「二重管理を避ける」原則を維持)。代わりに**gitブランチ**でバージョンとPublic/Private状態を表現する:

- 対象ノードは、node-library内で専用の安定ブランチを持つ(例: `node/<id>/stable`)。このブランチの指す先が「現在のPublicバージョン」。
- 新バージョンの開発は、別ブランチ(作業ブランチ)で行う。この時点ではまだ`stable`ブランチに反映されていないため、そのブランチを直接参照しているcontent repo(例: 開発元のTest3)以外には影響しない。これが「Private」の実体。
- **承認 = `stable`ブランチを新しいコミットへfast-forwardする操作。** これを人間が明示的に行う(自動化しない、CLAUDE.md「絶対に守ること」参照)。
- content repo側は、対象ノードを`vendor/node-library`全体のsubmoduleとは別に、**個別のsubmodule**として追加し(例: `vendor/nodes/<id>`)、`stable`ブランチを追跡する(`git submodule set-branch --branch node/<id>/stable`)。これにより、Test1・Test2は`stable`を、Test3は作業ブランチを、同時に別々に参照できる(node-library全体を1コミットに固定する2.5の仕組みだけでは、この「ノード単位で異なるバージョンを同時に参照する」状態を表現できないため)。
- **承認後の自動反映**: `stable`ブランチの更新をトリガーに、CIが「そのノードを個別submoduleとして追跡している全content repo」に対して`git submodule update --remote`→`verify.ts`実行→(成功すれば)コミット・pushを自動実行する。これはこのノードに限り2.9の「反映は必ず人が意図的にトリガーする」を「承認」という1回の操作に置き換えるものであり、対象をnon-security-sensitiveに限定することで、被害が取り返しのつかない領域にはこの簡略化を及ぼさない。
- **ダウングレード**: `stable`ブランチを古いコミットへ戻す操作を行えば、同じCIの仕組みにより全content repoへ自動的に反映される。

**必要になる新しいインフラ**(2.10の「追加インフラ不要」の原則から部分的に外れる点):
- node-library側のCIが、各content repoへの書き込み権限(Personal Access Token等)を持つ必要がある。このトークンの管理・スコープを絞る運用は、実装時に別途詰める。

**このステップの完了条件**(Phase 2.5、3章参照): 実際に1つのnon-security-sensitiveノードで、Private(作業ブランチ)→承認(stable進行)→複数content repoへの自動反映→ダウングレード、という一連の流れを実機で検証すること。
...
- **ノード横断のエラー契約が存在しない**: 配線スパイクで、`shared-auth`の`AuthError`・`stripe-webhook`の`StripeSignatureError`・`error-handling-convention`の`AppError`が互いに何の契約も共有していないことが判明した。複数ノードを組み合わせた際、個々のエラー種別情報が失われ、すべて`error-handling-convention`側で汎用的な500 INTERNAL_ERRORに丸められてしまう。ノードは個別には疎結合だが、複数ノードを跨いだ時に本当の意味でシステムとして疎結合に組み合わさるとは言えない状態。
  - **対応(実施済み)**: ノード同士のimportは増やさず、「エラーは`code`(string)と`status`(number)を持つ」という緩やかな形状の規約(ダックタイピング)を導入した。`StripeSignatureError`/`AuthError`に`code`/`status`を追加し、`error-handling-convention`の`normalizeError`がこの形状を検出して本来のstatus/codeを保持したまま正規化するよう変更した。scratchpadでの組み合わせテストで、以前は500に丸められていたエラーが正しいstatus/codeで返ることを確認済み。
- **ブランチ固定の脆さ**: Phase 0の実機検証時、`node-library`の実装がまだ`main`にマージされておらず作業ブランチにのみ存在したため、参照側で`-b <branch>`の明示指定が必要になり、実際に混乱・手戻りが発生した。
  - **対応(方針のみ、実行は見送り)**: 根本解決(作業ブランチを`main`へマージ)は、ユーザーの明示的な依頼があった時点で行う。それまではドキュメント・スクリプトの実行例で作業ブランチ名を明記し続ける現状の運用を継続する。
- **アクセス権限の分散管理**: `node-library`がprivateリポジトリである以上、ノードを使う新しいマシン・CI環境ごとにGitHub認証を都度用意する必要がある。単一ユーザー運用では大きな負担ではないが、「疎結合」を謳う設計が結局GitHub認証という1点に強く依存している点は認識しておく。
  - **対応(対応しない)**: 単一ユーザー運用の前提(0章)では実害が無く、install.sh経由の導入を実機で確認済み。追加の認証基盤は「実際に困ってから作る」の原則により見送る。
- **発見可能性(discoverability)の欠如**: ノード数が少ないうちは`ls nodes/`で足りるが、カタログやREADME一覧のような仕組みは無い。
  - **対応(実施済み)**: `nodes/README.md`に手書きの一覧表(id/node_type/category/forkable/一言説明)を追加した。自動生成はせず、`/add-node`の最後の手順として1行追記する運用にした(`.claude/skills/add-node/SKILL.md`参照)。本格的な視認性の仕組み(検索・ビジュアル一覧等)はPhase 3を待つ。

### Phase 2: 運用の定着
- 3つ以上のcontent repoで実際に使ってみる
- 一括反映を実際の必要(規約変更相当の事由)で最低1回実行してみる

### Phase 2.5: ノード単位のバージョニング配信システム(2.12参照)
- Phase 2で複数content repo運用の実績を積んだ後、反映コストが実際にボトルネックになった段階で着手
- non-security-sensitiveなノードを対象に、gitブランチによるバージョン管理(`stable`ブランチ)を導入
- content repo側が対象ノードを個別submoduleとして追加し、`stable`ブランチを追跡する仕組みを検証
- 承認(`stable`ブランチのfast-forward)をトリガーに、全content repoへ自動反映するCIを構築
- **完了条件**: 1つのnon-security-sensitiveノードで、Private→承認→複数content repoへの自動反映→ダウングレード、の一連の流れを実機検証すること
- security-sensitiveなノードにはこの仕組みを適用しない(2.12参照。ダウングレードは実害を取り消せないため)
...
| バージョニング配信システム(2.12)の自動反映で誤りが全content repoに即座に伝播する | 対象をnon-security-sensitiveなノードに限定。security-sensitiveなノードは2.9の手動反映モデルを維持し、ダウングレードでは取り消せない実害(不正取引・不正アクセス等)が発生するカテゴリには自動反映を及ぼさない |
| node-library側のCI用書き込みトークンが漏洩し、全content repoへの不正pushが可能になる | Phase 2.5着手時に、トークンのスコープを必要最小限(対象ノードのsubmoduleを追跡しているrepoのみ)に絞る運用を別途設計する |
...
- **バージョニング配信システム**(2.12): non-security-sensitiveなノードを対象に、gitブランチでバージョンとPublic/Private状態を管理し、承認をトリガーに全content repoへ自動反映する仕組み。Phase 2.5で導入予定。
- **stableブランチ**: バージョニング配信対象ノードの「現在のPublicバージョン」を指すブランチ。このブランチをfast-forwardする操作が「承認」にあたる。
- **Private(バージョニング配信システムにおける)**: まだ`stable`ブランチに反映されていない作業中のバージョン。開発元のcontent repoだけがそのブランチを直接参照して使える状態。
