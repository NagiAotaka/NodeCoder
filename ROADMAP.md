[ROADMAP.md](https://github.com/user-attachments/files/31332529/ROADMAP.md)
# ノード式ノーコードツール 設計書 兼 ロードマップ

## 0. 目的とスコープ

**目的**
**サイト・アプリ・デジタルコンテンツのコーディング全体を、なるべく多くの範囲でノードとして分散管理できるようにする。** これにより:

- 複数のコンテンツにまたがって構成をバラバラに管理できる
- システム的な側面(規約変更、認証ロジック、決済仕様の追従、エラーハンドリングやロギングの規約、APIのレスポンス形式、ルーティング規約など、コーディング全般に及ぶあらゆる共通部分)は全コンテンツに一括反映できる一方、各コンテンツ固有の状態(レイアウトの改変など)はそのコンテンツ内に留められる
- プレーンなファイル+gitの標準機能だけで成り立つため、将来Claude Code以外のコーディングエージェントに乗り換えても構成自体はそのまま引き継げる

**決済・認証はあくまで最初の実装例**であり、この設計の対象範囲そのものではない。決済・認証を最初に選んだのは、セキュリティ上の要求が最も厳しく、`forkable`禁止・動的verify.ts・シークレット管理といった安全機構を最も厳しい条件で検証できるためであって、design全体が決済・認証専用というわけではない。共通エラーハンドリング、ロギング規約、APIレスポンス形式、ルーティング規約、状態管理パターンなど、**システムコーディング全体のあらゆる繰り返し部分がノード化の対象になりうる**。
...
| 例(決済・認証以外も含む) | 利用規約テキスト、認証ロジック、決済Webhook処理、共通エラーハンドリング/ロギング規約、APIレスポンス形式 | ページの初期レイアウト、フォームの雛形、APIエンドポイントの雛形 |
...
│  ├─ error-handling-convention/     ← 参照型・非security-sensitive(決済・認証以外の例、Phase 0で下書き検証)
...
**スキーマが決済・認証専用になっていないかの確認**
`config`・`adapter_interface`はサービス連携系ノードにのみ使うオプション項目であり、必須ではない。共通エラーハンドリング規約・ロギング設定・APIレスポンス形式・ルーティング規約のような、外部サービスと無関係な「システムコーディング全般」のノードでは、これらを一切使わず`id` / `node_type` / `category` / `entry`(または`template`)だけで成立する。Phase 0では、決済・認証の2ノードを実装する前後に、こうした非サービス系ノードのschema.yamlを**実装せず下書きだけ**書いてみて、フィールドが決済・認証の形に引きずられていないかを確認する(3章Phase 0参照)。
...
- **参照型・security-sensitiveなノードを2つ作成**(決済Webhook処理、認証ロジック。ともに`forkable: false`)
- **スキーマ汎用性の下書き検証(実装なし)**: 決済・認証以外の参照型ノード(例: 共通エラーハンドリング/ロギング規約)のschema.yamlを、実装せず下書きだけ書いてみる。`config`/`adapter_interface`を使わずに`id`/`node_type`/`category`/`entry`だけで成立するかを確認し、決済・認証の形にスキーマが引きずられていないかをここで検知する。実装コストはかからない
...
- **生成型ノードを最低1つ作成**(例: ページレイアウトの叩き台、APIエンドポイントの雛形など、決済・認証以外の領域から選ぶ)し、GENERATED/CUSTOMIZEDマーカーの運用を検証。Phase 0の下書き検証(2.1)で確認した非サービス系ノードのschema.yamlを、ここで実際に実装まで進める
- **forkable: trueなノードを1つ作成しフォークを検証**(security-sensitiveでないもの。Phase 0で下書きした`error-handling-convention`などが候補)
...
- **完了条件**: 参照型/生成型の取り違え、フォーク禁止ルールの意図しない回避、が0件。加えて、決済・認証以外の領域でもノード化が無理なく機能することを確認する
...
| 設計・スキーマが決済・認証専用の形に固定化される | Phase 0で非サービス系ノードのschema.yaml下書き検証、Phase 1で実装まで進めて確認(2.1, 3章) |
...
- **参照型ノード**: 全コンテンツで中身が同じであり続けてほしいノード。決済・認証に限らず、エラーハンドリング規約やロギング設定なども対象。生成せずimportして使う。一括反映の対象。
