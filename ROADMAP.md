...
**目的**
サイト・アプリ・デジタルコンテンツ制作でよく使う構成を、疎結合な「ノード」として蓄積する。これにより:

- 複数のコンテンツにまたがって構成をバラバラに管理できる
- システム的な側面(規約変更、認証ロジック、決済仕様の追従など)は全コンテンツに一括反映できる一方、各コンテンツ固有の状態(レイアウトの改変など)はそのコンテンツ内に留められる
...
| 例 | 利用規約テキスト、認証ロジック、決済Webhook処理 | ページの初期レイアウト、フォームの雛形 |
...
- **生成型ノードを最低1つ作成**(例: ページレイアウトの叩き台)し、GENERATED/CUSTOMIZEDマーカーの運用を検証
- **forkable: trueなノードを1つ作成しフォークを検証**(security-sensitiveでないもの)
