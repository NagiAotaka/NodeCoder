...
### 具体的な再利用性テスト(ノード化する場合、必ず行う)

上の表で「ノードが向く」と判定されても、それだけでノード化してよいとは
**限らない**。「複数のコンテンツで使い回せる見込みがある」のような抽象的な
自己申告は不十分。以下を必ず自問する:

- **性質の異なる別プロジェクト(最低2つ)で、このまま(または軽微な設定
  変更だけで)使えると具体的にイメージできるか?** 「多分どこかで使える」
  ではなく、実際に異なる種類のプロジェクト名・用途を最低2つ挙げられるか
  を基準にする。挙げられないなら、そのcontent repo内に留めるべき機能。
- **あるアプリの中核機能をまるごとノード化していないか?** 1つのアプリの
  操作画面・インタラクションを丸ごとテンプレ化したものは、たとえ設定で
  タイトルやボタン文言を差し替えられても、実質的にはそのアプリ専用の
  機能である可能性が高い。まずそれを支える**もっと基礎的で汎用的な部品**
  に分解できないか検討する(例: ページレイアウト、汎用的なランダム選択
  ロジック、通知UI、フォームバリデーションなど)。汎用部品だけをノード化
  し、アプリ固有の組み合わせ方・見た目・文言はcontent repo側に残す。

**Good例**(特定アプリに依存しない汎用部品): `page-layout-basic`(サイトの
土台)、`error-handling-convention`、`shared-auth`、`stripe-webhook`

**Bad例**(1アプリ専用の機能をまるごとノード化): 「矢印を回してランダムな
角度で止める」というインタラクション全体を1つのノードにする、のように、
そのアプリの中核機能そのものを切り出しただけのもの。タイトル・ボタン文言
が設定可能でも、機能の核(このケースでは「矢印を回して止める」という
インタラクションそのもの)がその1アプリ専用である限り、汎用部品とは言えない。
