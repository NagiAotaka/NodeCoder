...
- 一括反映の自動化(cron等)→ **security-sensitiveなノードには今後も適用しない(常に手動トリガー)。non-security-sensitiveなノードに限り、Phase 2.5でバージョニング配信システム(ROADMAP.md 2.12)を通じた承認ベースの自動反映を導入する設計まで完了している(実装はまだ)**
- npm package化・workspace化 → 検討済みで不採用(ROADMAP.md 2.10参照)
- 3つ以上のcontent repoでの本格運用・定着 → Phase 2
- バージョニング配信システム(ROADMAP.md 2.12)の実装 → **設計は完了。実装はPhase 2.5**(Phase 2で複数content repo運用の実績を積み、反映コストが実際にボトルネックになってから着手)
...
- **バージョニング配信システム(ROADMAP.md 2.12、Phase 2.5)の自動反映は、security-sensitiveなノードには絶対に適用しない。** ダウングレードはコードの不具合を是正できても、露出していた間に発生した実害(不正な取引・不正アクセス等)は取り消せないため。
